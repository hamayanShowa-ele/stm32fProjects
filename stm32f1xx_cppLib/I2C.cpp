...
int STM32F_I2C::begin( I2C_TypeDef *i2c, uint8_t _sda, uint8_t _scl, uint32_t speed )
{
  ...
  if( wire == I2C1 )
  {
    rccClockEnable( RCC_I2C1 );
    if( sclPin == PB8 && sdaPin == PB9 )  /* scl:PB8 sda:PB9 REMAP */
    {
      rccClockEnable( RCC_GPIOB );  // RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOB, ENABLE );
      remap( REMAP_I2C1 );  // GPIO_PinRemapConfig( GPIO_Remap_I2C1 , ENABLE );  // If you are using PB8 and PB9 in I2C1, then you will need a REMAP.
    }
  ...
  else if( wire == I2C2 )  /* scl:PB10 sda:PB11 */
  {
    rccClockEnable( RCC_I2C2 );
    if( sclPin == PB10 && sdaPin == PB11 )  /* scl:PB10 sda:PB11 */
    {
      rccClockEnable( RCC_GPIOB );
    }
    else return I2C_DEFINITION_ERROR;
