...
//extern int _xwrite(int fd, char *str, int len);
...
int fprintf(FILE * stream, const char *fmt, ...)
{
	...
	va_end(va);
	{
		...
		va_start(va, fmt);
		length = ts_formatstring(buf, fmt, va);
		length = _xwrite(stream->_file, buf, length);
		va_end(va);
	}
	return length;
}
...
int printf(const char *fmt, ...)
{
	...
	va_end(va);
	{
		...
		va_start(va, fmt);
		length = ts_formatstring(buf, fmt, va);
		length = _xwrite(STDOUT, buf, length);
		va_end(va);
	}
	return length;
}
