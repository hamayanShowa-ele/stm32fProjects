...
/* ----------------------------------------
    begin and end
---------------------------------------- */
int STM32F_I2C::begin( I2C_TypeDef *i2c, uint8_t _sda, uint8_t _scl, uint32_t speed )
{
  /* initialize i2c bus. */
  /* save scl and sda pin numbers. */
  wire = i2c;
  sclPin = _scl; sdaPin = _sda;
  uint32_t rccSCL,rccSDA;
  uint8_t alterFunction;  /* alternate function. */

  if( wire == I2C1 )
  {
    RCC_APB1PeriphClockCmd( RCC_APB1Periph_I2C1, ENABLE );
    RCC_APB1PeriphResetCmd( RCC_APB1Periph_I2C1, DISABLE );
    alterFunction = GPIO_AF_I2C1;
    if( sclPin == PB8 && sdaPin == PB9 )  /* scl:pb8,sda:pb9 */
    {
      rccSCL = RCC_AHB1Periph_GPIOB;
      rccSDA = RCC_AHB1Periph_GPIOB;
    }
    else if( sclPin == PB6 && sdaPin == PB7 )  /* scl:pb6,sda:pb7 */
    {
      rccSCL = RCC_AHB1Periph_GPIOB;
      rccSDA = RCC_AHB1Periph_GPIOB;
    }
    else return I2C_DEFINITION_ERROR;
  }
  else if( wire == I2C2 )
  {
    RCC_APB1PeriphClockCmd( RCC_APB1Periph_I2C2, ENABLE );
    RCC_APB1PeriphResetCmd( RCC_APB1Periph_I2C2, DISABLE );
    alterFunction = GPIO_AF_I2C2;
    if( sclPin == PF1 && sdaPin == PF0 )  /* scl:pf1,sda:pf0 */
    {
      rccSCL = RCC_AHB1Periph_GPIOF;
      rccSDA = RCC_AHB1Periph_GPIOF;
    }
    else if( sclPin == PH4 && sdaPin == PH5 )  /* scl:ph4,sda:ph5 */
    {
      rccSCL = RCC_AHB1Periph_GPIOH;
      rccSDA = RCC_AHB1Periph_GPIOH;
    }
    else if( sclPin == PB10 && sdaPin == PB11 )  /* scl:pb10,sda:pb11 */
    {
      rccSCL = RCC_AHB1Periph_GPIOB;
      rccSDA = RCC_AHB1Periph_GPIOB;
    }
  ...
  else if( wire == I2C3 )
  {
    ...
    alterFunction = GPIO_AF_I2C3;
    if( sclPin == PH7 && sdaPin == PH8 )  /* scl:ph7,sda:ph8 */
    {
      rccSCL = RCC_AHB1Periph_GPIOH;
      rccSDA = RCC_AHB1Periph_GPIOH;
    }
    else if( sclPin == PA8 && sdaPin == PC9 )  /* scl:pa8,sda:pc9 */
    {
      rccSCL = RCC_AHB1Periph_GPIOA;
      rccSDA = RCC_AHB1Periph_GPIOC;
    }
    else return I2C_DEFINITION_ERROR;
  }
  else return I2C_DEFINITION_ERROR;
  ...
  /* RCC configuration. */
  RCC_AHB1PeriphClockCmd( rccSCL, ENABLE );
//  RCC_AHB1PeriphResetCmd( rccSCL, DISABLE );
  RCC_AHB1PeriphClockCmd( rccSDA, ENABLE );
//  RCC_AHB1PeriphResetCmd( rccSDA, DISABLE );

  /* clear i2c bus. */
  if( pinsAlternate() < 0 ) return I2C_BUS_BUSY;

  /* alternate function set. */
  pinMode( sclPin, ALTERNATE_OD, GPIO_SPEED_FAST );
  pinMode( sdaPin, ALTERNATE_OD, GPIO_SPEED_FAST );
  pinAFConfig( sclPin, alterFunction );
  pinAFConfig( sdaPin, alterFunction );
