/* ----------------------------------------
 stm32f4xx Timer utility header
...
/* ----------------------------------------
    defines
---------------------------------------- */
enum CK_INT_TYPE { ADVANCED_CONTROL_TIMER, GENERAL_PURPOSE_TIMER1, GENERAL_PURPOSE_TIMER2, BASIC_TIMER, };
...
private:
  ...
  void config( uint16_t prescaler, uint32_t period );
  ...
  int  altFunc( int pin );
  int  OC( int ch, uint16_t ocMode, int pin, uint32_t pulse, uint16_t pol, uint16_t rst );
...
public:
  ...
  int  period( double peri );

  volatile uint32_t getCounter();
  void setAutoReload( uint16_t reload );
  volatile uint32_t getAutoReload();
  void master();
  void slave( uint16_t parent, uint16_t prescaler, uint32_t period );

  int  toggle( int ch, int pin = 0 );
  int  pwm1( int ch, int pin, uint32_t pulse );
  int  adcTrigger( int ch, int pin, uint32_t pulse );
  void setPulse( int ch, uint32_t pulse );
  uint32_t getPulse( int ch );
